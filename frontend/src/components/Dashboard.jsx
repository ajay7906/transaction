...
export default function Dashboard() {
  ...
  useEffect(() => {
    // Listen for real-time transaction updates for the logged-in user
    socket.on("newTransaction", (data) => {
      // Check if the current user is the receiver
      if (data.receiver === userIds) {
        setTransactionNotification(data);
      }
    });
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      {/* Real-Time Transaction Notification Popup */}
      {transactionNotification && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded shadow-lg w-96">
            <h2 className="text-xl font-bold mb-4">Transaction Received!</h2>
            <p className="mb-4">
              You have received {transactionNotification.amount} from{" "}
              {transactionNotification.sender.name}.
            </p>
            <button
              onClick={() => setTransactionNotification(null)}
              className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600"
            >
              Close
            </button>
